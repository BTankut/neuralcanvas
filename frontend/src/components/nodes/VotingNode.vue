...
<template>
  ...
  >
    ...
    <!-- Expand Button -->
    <button v-if="streamText" @click="isExpanded = true" class="absolute top-2 right-2 text-slate-500 hover:text-neon-orange transition-colors z-30">
    ...
    <div class="node-header bg-neon-orange/20 border-b border-neon-orange/30 text-neon-orange flex justify-center items-center min-h-[40px] relative pl-6">
        ...
        <span v-if="status.status === 'success'" class="absolute right-2 text-[9px] font-bold text-neon-green">VERDICT</span>
    </div>
    ...
    <div class="node-body p-3 space-y-3">
        ...
        <div v-else class="space-y-3">
            <div>
                ...
                <select v-model="model" class="w-full bg-black/50 border border-slate-700 rounded text-xs text-slate-200 p-1 focus:border-neon-orange outline-none">
