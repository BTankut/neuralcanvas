...
<template>
  <div 
    class="neural-node-base conditional-node w-64 relative transition-all duration-300"
    :class="{
  ...
  >
    <!-- Header -->
    <div class="node-header bg-neon-yellow/20 border-b border-neon-yellow/30 text-neon-yellow flex justify-between items-center">
        <div class="flex items-center">
            <PhGitFork weight="bold" class="text-lg mr-2" />
            <span class="font-bold text-xs tracking-wider uppercase">Condition</span>
        </div>
        <!-- Status Dot -->
        <div v-if="status.status === 'success'" class="px-2 py-0.5 rounded text-[9px] font-bold uppercase"
