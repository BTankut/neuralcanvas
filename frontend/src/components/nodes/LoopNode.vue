...
<template>
  <div 
    class="neural-node-base loop-node w-64 relative transition-all duration-300"
    :class="{
  ...
  >
    <!-- Header -->
    <div class="node-header bg-neon-cyan/20 border-b border-neon-cyan/30 text-neon-cyan flex justify-between items-center">
        <div class="flex items-center">
            <PhArrowsClockwise weight="bold" class="text-lg mr-2" :class="{'animate-spin': isRunning}" />
            <span class="font-bold text-xs tracking-wider uppercase">Iterator</span>
        </div>
        <div v-if="isRunning || status.status === 'success'" class="px-2 py-0.5 bg-black/50 rounded text-[9px] font-mono border border-neon-cyan/50">
